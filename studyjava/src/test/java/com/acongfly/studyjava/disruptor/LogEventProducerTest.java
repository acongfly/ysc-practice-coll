...
/**
 * 参考：https://357029540.iteye.com/blog/2395677
 */
public class LogEventProducerTest {
    ...
    public void producer() throws InterruptedException {
        ...
        // 定义Disruptor初始化信息
        Disruptor<LogEvent> disruptor = new Disruptor<LogEvent>(logEventFactory, ringBufferSize,
    ...
    /**
     * 一个生产者，3个消费者，其中前面2个消费者完成后第3个消费者才可以消费 也即使说当前面2个消费者把所有的RingBuffer占领完成，同时都消费完成后才会有第3个消费者的消费
    ...
    public void multiConsumer() throws InterruptedException {
        ...
        EventHandlerGroup<LogEvent> eventEventHandlerGroup =
            disruptor.handleEventsWith(new LogEventConsumer(), new LogEventConsumer2());
    ...
    /**
     * 一个生产者，多个消费者，有2条支线，其中消费者1和消费者3在同一条支线上， 消费者2和消费者4在同一条支线上，消费者5是消费者3和消费者4的终点消费者
     * 这样的消费将会在消费者1和消费者2把所有的RingBuffer大小消费完成后才会执行消费者3和消费者4 在消费者3和消费者4把RingBuffer大小消费完成后才会执行消费者5
     * 消费者5消费完RingBuffer大小后又按照上面的顺序来消费 如果剩余的生产数据比RingBuffer小，那么还是要依照顺序来
     *
     * @throws InterruptedException
     */
    @Test
    ...
    /**
     * 多个生产者，多个消费者，有2条消费者支线，其中消费者1和消费者3在同一条支线上， 消费者2和消费者4在同一条支线上，消费者5是消费者3和消费者4的终点消费者
     * 这样的消费将会在消费者1和消费者2把所有的RingBuffer大小消费完成后才会执行消费者3和消费者4 在消费者3和消费者4把RingBuffer大小消费完成后才会执行消费者5
     * 消费者5消费完RingBuffer大小后又按照上面的顺序来消费 如果剩余的生产数据比RingBuffer小，那么还是要依照顺序来 生产者只是多生产了数据
     *
     * @throws InterruptedException
     */
    @Test
