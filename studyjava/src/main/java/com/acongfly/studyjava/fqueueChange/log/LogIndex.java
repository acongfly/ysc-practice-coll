 ...
 * @date 2011-5-18
 */
public class LogIndex implements Closeable {
    ...
    public LogIndex(String path) throws IOException, FileFormatException {
        ...
        if (!dbFile.exists()) {
            if (!dbFile.createNewFile()) {
                LOGGER.error("创建文件失败:=>file:[{}]", dbFile.getAbsolutePath());
            }
            dbRandFile = new RandomAccessFile(dbFile, "rwd");
            dbRandFile.write(LogEntity.MAGIC_BYTES);// magic
            ...
            if (dbRandFile.read(b) == -1) {
                throw new IOException("can not read files");
            }
            ByteBuffer buffer = ByteBuffer.wrap(b);
            b = new byte[LogEntity.MAGIC_BYTES.length];
            buffer.get(b);
            magicString = new String(b, LogEntity.MAGIC_CHARSET);
     ...
     * @param pos
     */
    void putWriterPosition(int pos) {
     ...
     * @param pos
     */
    void putReaderPosition(int pos) {
    ...
    public void decrementSize(int num) {
        num = size.addAndGet(0 - num);
        mappedByteBuffer.position(28);
        mappedByteBuffer.putInt(num);
    }
    ...
    public String toString() {
        final StringBuilder sb = new StringBuilder("LogIndex{");
