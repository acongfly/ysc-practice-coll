...
/**
 * @author shicongyang
 * @Description: ArrayBlockingQueue 基于数组的阻塞队列实现，在 ArrayBlockingQueue内维护了一个定长数组，以便缓存队列中的数据对象
 *               其内部没实现读写分离，ArrayBlockingQueue则读写共享一个锁，意味着生产和消费不能完全并行，长度是需要定义的， 可以指定先进先出或者先进后出，也叫有界队列，在很多场合非常适合适用
 * @date 2018/6/6 9:52
 */
public class ArrayBlockingQueueStudy {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            /**
             * add() 在不超出队列长度的情况下插入元素，可以立即执行，成功返回true，如果队列满了就抛出异常 其底层实现的是offer方法，不会阻塞 public boolean add(E e) { if
             * (offer(e)) return true; else throw new IllegalStateException("Queue full"); }
             */
            boolean add = arrayBlockingQueue.add(i + "");
            System.out.println(add);
            /**
             * offer（）方法： 在不超出队列长度的情况下插入元素的时候则可以立即在队列的尾部插入指定元素 成功时返回true，如果此队列已满，则返回false。 不会阻塞
             *
             */
            // boolean offer = arrayBlockingQueue.offer(i + "");
            // System.out.println(offer);
            /**
             * put()方法： 插入元素的时候，如果队列满了就进行等待，直到队列可用
             */
            // try {
        ...
        for (int i = 0; i < 5; i++) {
            /**
             * remove（）方法取值： 底层是用到了poll()方法，检索并且删除返回队列头的元素 与poll()方法不同的是，元素没有是进行抛异常NoSuchElementException public E
             * remove() { E x = poll(); if (x != null) return x; else throw new NoSuchElementException(); }
             */
