...
public class StreamStudy01 {
    ...
    public static void fiterSexAndAge() {
        ...
        List<PersonModel> collect1 =
            data.stream().filter(person -> ("男".equals(person.getSex()) && person.getAge() < 20)).collect(toList());

    }

    /**
     * Map map生成的是个一对一映射,for的作用 比较常用 而且很简单
     */
    /**
     * 取出所有的用户名字
     */
    ...
    /**
     * 顾名思义，跟map差不多,更深层次的操作 但还是有区别的 map和flat返回值不同 Map 每个输入元素，都按照规则转换成为另外一个元素。 还有一些场景，是一对多映射关系的，这时需要 flatMap。 Map一对一
     * Flatmap一对多 map和flatMap的方法声明是不一样的 <r> Stream<r> map(Function mapper); <r> Stream<r> flatMap(Function> mapper);
    ...
    public static void flatMapString() {
        ...
        // 返回类型不一样
        List<String> collect =
            data.stream().flatMap(person -> Arrays.stream(person.getName().split(" "))).collect(toList());

        List<Stream<String>> collect1 =
            data.stream().map(person -> Arrays.stream(person.getName().split(" "))).collect(toList());

        // 用map实现
        List<String> collect2 =
            data.stream().map(person -> person.getName().split(" ")).flatMap(Arrays::stream).collect(toList());
        // 另一种方式
    ...
    public static void toMapTest() {
        ...
        Map<String, Integer> collect =
            data.stream().collect(Collectors.toMap(PersonModel::getName, PersonModel::getAge));

        data.stream().collect(Collectors.toMap(per -> per.getName(), value -> {
            return value + "1";
        }));
    }
    ...
    public static void toGroupTest() {
        List<PersonModel> data = Data.getData();
        Map<Boolean, List<PersonModel>> collect =
            data.stream().collect(Collectors.groupingBy(per -> "男".equals(per.getSex())));
        System.out.println(collect);
    }
    ...
    public static void toJoiningTest() {
        List<PersonModel> data = Data.getData();
        String collect =
            data.stream().map(personModel -> personModel.getName()).collect(Collectors.joining(",", "{", "}"));
        System.out.println(collect);
    }
     ...
     * 用处很广，不光在lambda中，哪都能用 Optional.of(T)，T为非空，否则初始化报错 Optional.ofNullable(T)，T为任意，可以为空 isPresent()，相当于 ！=null
