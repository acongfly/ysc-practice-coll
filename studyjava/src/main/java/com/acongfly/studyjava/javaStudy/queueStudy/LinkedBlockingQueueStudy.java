...
/**
 * @author shicongyang
 * @Description: LinkedBlockingQueue 基于链表的阻塞队列，同 ArrayBlockingQueue类似，其内部也维持着一个数据缓冲队列 （该队列由一个链表构成），
 *               LinkedBlockingQueue之所以可以高效的处理并发数据，是因为其内部实现采用分离锁 （读写分离两个锁），从而实现生产者和消费者操作的完全并行运行，它是一个无界队列
...
public class LinkedBlockingQueueStudy {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 6; i++) {
            /**
             * 如果可以立即执行而不违反容量限制，则将指定的元素插入此队列，成功时返回true， 如果当前没有空间，则返回IllegalStateException
             */
