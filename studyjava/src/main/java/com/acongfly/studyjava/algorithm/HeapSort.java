 ...
 * 算法的基本描述：
 * 堆积近似是一个完全二叉树的结构：
 * 1.将初始待排序关键字的无序的序列（A1,A2,A3,...,An）构建成大顶锥，现在这堆为无序的；
 * 2.将堆顶元素A[1]与最后一个元素A[n]交换，此时得到的是一个新的无序区(A1,A2,A3,...,An-1)和
 * 新的有序区An,切满足A[1,2,3,...,n-1]<=A[n];
 * 3.由于交换后的堆顶A[1],可能违反堆的性质，所以需要对当前无序区（A1,A2,...,An-1）调整为新堆，然后
 * 再次将A[1]与无序区最后一个元素交换，得到新的无序区（A1,A2,...,An-2）和新的有序区（An-1,An）,不断重复此过程直到有序区
 * 的元素个数为n-1，那么整个排序过程完成
