 ...
 * @Description: 计数排序 平均时间复杂度O(n+k) 最好情况：O(n+k) 最坏情况：O(n+k) 空间复杂度：O(k) 此算法占用额外的内存 是稳定的排序算法 算法的基本描述： 1.找出待排序的数组中的最大和最小的元素
 *               2.统计出数组中的每个值的元素出现的次数，存入数组c的第i项； 3.对所有的计数累加（从c中的第一个元素开始，每一项和前一项相加）；
