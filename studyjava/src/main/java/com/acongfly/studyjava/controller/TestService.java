...
//        File file = FileUtil.writeBytes(content, new File("/Users/shicongyang/work/testWrite2.csv"));
    ...
    public static ExecutorService paymentToEngine = new ThreadPoolExecutor(Runtime.getRuntime().availableProcessors() * 2, Runtime.getRuntime().availableProcessors() * 4,
            2, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder().setNameFormat("payment-engine-%d").build(), new ThreadPoolExecutor.CallerRunsPolicy());
