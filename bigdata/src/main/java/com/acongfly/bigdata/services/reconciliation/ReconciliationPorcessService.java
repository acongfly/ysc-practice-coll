...
public class ReconciliationPorcessService {
    ...
    enum FILTER_TYPE {
        DB_NOT_EXIST, WX_NOT_EXIST, EQUAL_DATA
    }
    ...
    private static final String[] DB_BILL_SOURCE = {"merchantId", "tradeMasterNo", "merchantBatchNo", "orderId",
        "tradeNo", "status", "amount", "currency", "countryCode", "payType", "payeeUpi", "payeeName"

    };
    private static final String[] DB_BILL_SOURCE_WX =
        {"wx_merchantId", "wx_tradeMasterNo", "wx_merchantBatchNo", "wx_orderId", "wx_tradeNo", "wx_status",
            "wx_amount", "wx_currency", "wx_countryCode", "wx_payType", "wx_payeeUpi", "wx_payeeName"

        };

    Fields baseField = new Fields("merchantId", "tradeMasterNo", "merchantBatchNo", "orderId", "tradeNo", "status",
        "amount", "currency", "countryCode", "payType", "payeeUpi", "payeeName");

    private static final String[] DB_SUM_SOURCE = {"sumAmt", "sumSize"};
    ...
    public void creatTxnCsv() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            a = new String[] {result.getMerchantId(), result.getTradeMasterNo(), result.getMerchantBatchNo(),
                result.getOrderId(), result.getTradeNo(), result.getStatus().toString(), result.getAmount().toString(),
                result.getCurrency(), result.getCountryCode(), result.getPayType(), result.getPayeeUpi(),
                result.getPayeeName()};
            objects.add(a);
        }
    ...
    public void sumProcess() throws IOException {
        ...
        Properties properties = AppProps.appProps().setName(ReconciliationPorcessService.class.getSimpleName() + "App")
            .setVersion("0.1").buildProperties(jobConf);
        ...
        Pipe allJoin = new CoGroup(filterQuota, new Fields("wx_orderId", "wx_merchantBatchNo"), filterDbPipe,
            new Fields("orderId", "merchantBatchNo"), payOrderJoinFields, new OuterJoin());
        // 0.2.1 DB不存在的记录
        ...
        Tap wxNotTap = new Hfs(new TextDelimited(baseField, true, ","), String.format("%s/%s", targetDir, WX_NOT_PATH),
            SinkMode.REPLACE);
        Tap dbNotTap = new Hfs(new TextDelimited(baseField, true, ","), String.format("%s/%s", targetDir, DB_NOT_PATH),
            SinkMode.REPLACE);
        Tap equalTap = new Hfs(new TextDelimited(baseField, true, ","), String.format("%s/%s", targetDir, EQUAL_PATH),
            SinkMode.REPLACE);
